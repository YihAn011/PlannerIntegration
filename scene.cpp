...
: SceneModuleInterface{name, node, rtc_interface_ptr_map, objects_of_interest_marker_interface_ptr_map},  // NOLINT
  ...
  target_objects_manager_{TargetObjectsManager(
    ...
    //new planner
  // xdwa_local_planner_{std::make_shared<xdwa_local_planner::XDWAPlanner>(node)}
   xdwa_local_planner_{std::make_shared<xdwa_local_planner::XDWALocalPlanner>(node)}
...
//new planner
  ...
  std::shared_ptr<xdwa_local_planner::Trajectory> best_traj;
  if (xdwa_local_planner_->computeBestTrajectory(best_traj)) {
    //convert xdwa_local_planner_ format to autoware format
    //adding some logs INFO
    output.path.header.stamp = node_.get_clock()->now(); 
    output.path.header.frame_id = "map"; 
    for (int i = 0; i < best_traj->num_points_; i++) {
      autoware_auto_planning_msgs::msg::PathPointWithLaneId path_point; 
      path_point.point.pose.position.x = best_traj->x_[i];
      path_point.point.pose.position.y = best_traj->y_[i];
      path_point.point.pose.position.z = best_traj->z_[i];
      output.path.points.push_back(path_point);
    }
    
  } else {
    RCLCPP_WARN(getLogger(), "XDWA Local Planner failed to produce a valid path.");
    // if new planner does not work, use the original planner
    //output.path = input_path; //for test reason, force it to use new planner
  }
